...
class ChainWalletBase extends wallet_1.WalletBase {
    ...
    async update(options = { connect: true }) {
        ...
        if (!this.isWalletRejected && this.walletName !== types_1.IFRAME_WALLET_ID) {
            window?.localStorage.setItem('cosmos-kit@2:core//current-wallet', this.walletName);
        }
    }
    getRpcEndpoint = async (isLazy) => {
        const lazy = (0, utils_1.getIsLazy)(void 0, this.isLazy, this._rpcEndpoint?.isLazy, isLazy, this.logger);
        if (lazy) {
        ...
        if (this._rpcEndpoint &&
            (await (0, utils_1.isValidEndpoint)(this._rpcEndpoint, nodeType, lazy, this.logger))) {
            return this._rpcEndpoint;
        }
        this._rpcEndpoint = await (0, utils_1.getFastestEndpoint)(this.rpcEndpoints, nodeType, this.logger);
        return this._rpcEndpoint;
    };
    getRestEndpoint = async (isLazy) => {
        const lazy = (0, utils_1.getIsLazy)(void 0, this.isLazy, this._restEndpoint?.isLazy, isLazy, this.logger);
        if (lazy) {
        ...
        if (this._restEndpoint &&
            (await (0, utils_1.isValidEndpoint)(this._restEndpoint, nodeType, lazy, this.logger))) {
            return this._restEndpoint;
        }
        this._restEndpoint = await (0, utils_1.getFastestEndpoint)(this.restEndpoints, nodeType, this.logger);
        return this._restEndpoint;
    };
    getStargateClient = async () => {
        const rpcEndpoint = await this.getRpcEndpoint();
        return stargate_1.StargateClient.connect(rpcEndpoint, this.stargateOptions);
    };
    ...
    getNameService = async () => {
        ...
        const registry = (0, utils_1.getNameServiceRegistryFromChainName)(this.chainName);
        return new name_service_1.NameService(client, registry);
    };
    ...
    getSigningStargateClient = async () => {
        ...
        return stargate_1.SigningStargateClient.connectWithSigner(rpcEndpoint, this.offlineSigner, this.signingStargateOptions);
    };
    getSigningCosmWasmClient = async () => {
        ...
        return cosmwasm_stargate_1.SigningCosmWasmClient.connectWithSigner(rpcEndpoint, this.offlineSigner, this.signingCosmwasmOptions);
    };
    ...
    estimateFee = async (messages, type, memo, multiplier) => {
        ...
        return (0, stargate_1.calculateFee)(Math.round(gasEstimation * (multiplier || 1.3)), gasPrice);
    };
    ...
    broadcast = async (signedMessages, type) => {
        const client = await this.getSigningClient(type);
        const txBytes = tx_1.TxRaw.encode(signedMessages).finish();
