...
class WalletManager extends bases_1.StateBase {
    ...
    disableIframe;
    _reconnectMap = {};
    constructor(chains, assetLists, wallets, logger, throwErrors = false, subscribeConnectEvents = true, disableIframe = false, defaultNameService, walletConnectOptions, signerOptions, endpointOptions, sessionOptions) {
        super();
    ...
    init(chains, assetLists, wallets, walletConnectOptions, signerOptions, endpointOptions) {
        ...
        this.chainRecords = chains.map((chain) => (0, utils_1.convertChain)(chain, assetLists, signerOptions, endpointOptions?.endpoints?.[chain.chain_name], this.isLazy, this.logger));
    ...
    addChains = (chains, assetLists, signerOptions, endpoints) => {
        const newChainRecords = chains.map((chain) => (0, utils_1.convertChain)(chain, assetLists, signerOptions, endpoints?.[chain.chain_name], this.isLazy, this.logger));
    ...
    getNameService = async (chainName) => {
        ...
        if (!chainName) {
            ...
            const registry = (0, utils_1.getNameServiceRegistryFromName)(this.defaultNameService);
            if (!registry) {
    ...
    _reconnectToIframe = () => this._reconnect(types_1.IFRAME_WALLET_ID);
    onMounted = async () => {
        ...
        if (!this.disableIframe && window.top !== window.self) {
            window.addEventListener(types_1.IFRAME_KEYSTORECHANGE_EVENT, this._reconnectToIframe);
        }
    ...
    onUnmounted = () => {
        ...
        if (!this.disableIframe && window.top !== window.self) {
            window.removeEventListener(types_1.IFRAME_KEYSTORECHANGE_EVENT, this._reconnectToIframe);
        }
        this.mainWallets.forEach((wallet) => {
            wallet.walletInfo.connectEventNamesOnWindow?.forEach((eventName) => {
