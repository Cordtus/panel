...
import { IFRAME_KEYSTORECHANGE_EVENT, IFRAME_WALLET_ID, State, } from './types';
...
export class WalletManager extends StateBase {
    ...
    onMounted = async () => {
        ...
        if (!this.disableIframe && window.top !== window.self) {
            window.addEventListener(IFRAME_KEYSTORECHANGE_EVENT, this._reconnectToIframe);
        }
    ...
    onUnmounted = () => {
        ...
        if (!this.disableIframe && window.top !== window.self) {
            window.removeEventListener(IFRAME_KEYSTORECHANGE_EVENT, this._reconnectToIframe);
        }
        this.mainWallets.forEach((wallet) => {
            wallet.walletInfo.connectEventNamesOnWindow?.forEach((eventName) => {
